...
) : Message
{
    ...
    // argument array per call and dominated CPU on element-heavy reads. It also
    // tripped a .NET 8 tier-up bug: after the reflection path was Tier1-recompiled
    // (i.e. after the call site became hot) it would null-deref deep in the
    // InvokeStub. Routing through a typed delegate built once per element type
    // avoids both problems: the reflection is paid once at cache-miss time, and
    // the per-element call is a direct invocation of the generic
    // GetDecodeInfoForUnmanagedElement<T> with no MethodInfo.Invoke on the hot path.
    ...
    // Avoid MemoryMarshal.AsBytes here. With this delegate cached and called on
    // every unmanaged read, AsBytes' checked(span.Length * sizeof(T)) was observed
    // to intermittently throw OverflowException on .NET 8 even when the inputs
    // cannot overflow (e.g. Length=1, sizeof(T)=12); disabling tiered
    // compilation eliminated the throw. Building the byte Span directly sidesteps
    // the throw site.
